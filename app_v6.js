...
function disconnectWallet() {
    ...
    document.getElementById('wallet-balance').textContent = "0 WORLD";
...
async function updateWalletInfo() {
    ...
    try {
        if (window.ethereum) {
            ...
            document.getElementById('wallet-balance').textContent = `${balanceInEth.toFixed(4)} WORLD`;
        }
    } catch (error) {
...
async function checkCorrectNetwork() {
    ...
    if (!isBNBTestnet(currentChainId)) {
        ...
        console.log("Rede correta detectada: WORLD Chain Testnet");
        // Limpar alertas anteriores sobre rede incorreta
        const alerts = document.querySelectorAll('.alert');
        alerts.forEach(alert => {
            if (alert.textContent.includes("WORLD Chain Testnet")) {
                alert.remove();
            }
        });
    }
        ...
        nftCard.innerHTML = `
           <div class="nft-card">
                ...
                <div class="nft-image-container">
                    ...
                    <div class="network-badge">WORLD Testnet</div>
                </div>
                <div class="nft-body">
